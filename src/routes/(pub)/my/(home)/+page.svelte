<script>
  ...
  import { updater, checkForUpdate } from "$lib/updater.svelte.js";
  ...
  {#if DATAS.isTauri}
    ...
    >
      ...
      >
        <span class="flex-cc gap-2">
          {#if updater.checking}
            <span class="i-line-md-loading-twotone-loop animate-spin text-green"
            ></span>
          {:else}
            <span class="i-carbon-update-now text-green"></span>
          {/if}
          检查更新
        </span>
